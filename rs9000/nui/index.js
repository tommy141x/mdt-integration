// Modified by Tommy Johnston // CREATED BY randomsean
// Tommy Johnston does not claim any rights to this resource.

//Your CAD url link with a trailing slash, example (https://cad.example.com/)
var communityapi = "PUT YOUR LINK HERE";
...
function init(data) {
  ...
  settings.plateReader =
    data.plateReader === undefined ? false : data.plateReader;
}
...
function setPlateDisplay(antennaName, plate) {
  ...
  if (value != undefined) {
    //RUN ALPR CODE
    $.getJSON(
      communityapi + "api/alpr.php?steam=" + steamid + "&plate=" + value.trim(),
      function (query) {
        owner = query.owner;
        flag = query.flag;
        exists = query.exists;
...
function sendMessage(action, data) {
  ...
  $.post(
    "http://" + settings.resourceName + "/" + action,
    JSON.stringify(body),
    function (response) {
